...
test('can edit a todo note', async ({ page }) => {
  ...
  // The note is open in edit mode
  await expect(page.getByTestId('note-title-input')).toBeVisible()
  await expect(page.getByTestId('note-todo-done-input')).toBeVisible()
  await expect(page.getByTestId('note-todo-text-input')).toBeVisible()
  await expect(page.getByTestId('save-note-btn')).toBeVisible()
  await expect(page.getByTestId('note-title-input')).toHaveValue(title)
  await expect(page.getByTestId('note-todo-text-input')).toHaveValue(todo1text)
  ...
  await page.getByTestId('note-todo-text-input').fill(todo1text_edited)
  ...
  await page.reload()
  await expect(page.getByTestId('note-title-input')).toHaveValue(title_edited)
  await expect(page.getByTestId('note-todo-text-input')).toHaveValue(todo1text_edited)
